...
module.exports = function() { //Models is a REST parameter.
    ...
    const verboseMode = settings.verbose_log_enabled;
    ...
    let customRoutes = '';
    if(settings.custom_routes_enabled === true){
        customRoutes  = require(path.resolve('./', settings.custom_routes_file));
    }
    ...
    app.post('/jwt-login', (req, res) => {
        ...
        if(currentCredential){
            ...
            res.status(401).send({ success: false, message: currentLang.jwt.user_pass_error });
        }
    });
    ...
    Object.entries(settings.models).forEach(([key, value]) => {
        ...
        app.use('/' + key, crudRoutes(currentModel, currentLang, verboseMode));
    });
