...
const checkJWT = (req, res, next) => {
    ...
    if(token){
      jwt.verify(token, settings.secret_token, (err, decoded) => {
        if(err){
          return res.json({ success: false, message: currentLang.jwt.check_invalid_token, error: err.message });
        } else {
          req.decoded = decoded;
          next();
        }
